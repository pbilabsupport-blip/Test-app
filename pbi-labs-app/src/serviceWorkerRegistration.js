      ...
      installingWorker.onstatechange = () => {
        if (installingWorker.state === 'installed') {
          if (navigator.serviceWorker.controller) {
            // New update available - auto refresh to prevent old cache loops
            window.location.reload();
          } else {
            console.log('P.B.I. Labs Vault is cached for offline use.');
          }
        }
      };
    };
  }).catch((error) => console.error('PWA Engine Error:', error));
}
